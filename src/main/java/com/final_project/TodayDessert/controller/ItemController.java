...
import com.final_project.TodayDessert.service.ItemImgService;
...
public class ItemController {
    ...
    @GetMapping(value = "/store/item/new")
    public String itemForm(Model model) {
        model.addAttribute("itemFormDto", new ItemFormDto());
        return "item/itemRegisterForm";
    }

    @PostMapping(value = "/store/item/new")
    ...
    @GetMapping(value = "/store/item/{itemId}")
    ...
    @PostMapping(value = "/store/item/{itemId}")
    ...
    @GetMapping(value = {"/store/items", "/store/items/{page}"})
    public String itemManage(ItemSearchDto itemSearchDto, @PathVariable("page") Optional<Integer> page, Model model){

        Pageable pageable = PageRequest.of(page.isPresent() ? page.get() : 0, 3);
        Page<Item> items = itemService.getStoreItemPage(itemSearchDto, pageable);
